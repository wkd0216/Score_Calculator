...
export function CourseList() {
  const { courses, removeCourse } = useGradeStore();

  if (courses.length === 0) {
    return (
      <div className="text-center py-12 bg-white/50 dark:bg-slate-900/50 rounded-xl border border-dashed border-muted-foreground/30 mt-6">
        ...
        <p className="text-sm text-muted-foreground/60">위 버튼을 사용하여 과목을 추가해 보세요!</p>
      </div>
    );
  }

  return (
    <div className="mt-6 bg-white/50 dark:bg-slate-900/50 rounded-xl shadow-sm border border-border/50 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Table>
        ...
        <TableBody>
          {courses.map((course) => (
            <TableRow key={course.id} className="group hover:bg-muted/10 dark:hover:bg-slate-800/20 transition-colors">
